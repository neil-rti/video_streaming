...
case ${TERM} in
  lxterminal)
    ${TERM} ${GEOMETRY} -t ffmpeg -e "ffmpeg -re -i RTI-vehicles-captioned-480.mp4 -g 15 -pix_fmt yuv420p -vcodec libx264 -preset ultrafast -tune zerolatency -f mpegts udp://127.0.0.1:2277" &
    ${TERM} ${GEOMETRY} -t ffplay -e "ffplay -fflags nobuffer -i udp://127.0.0.1:2277" &
    ;;
  xterm)
    ${TERM} ${GEOMETRY} -e "ffmpeg -re -i RTI-vehicles-captioned-480.mp4 -g 15 -pix_fmt yuv420p -vcodec libx264 -preset ultrafast -tune zerolatency -f mpegts udp://127.0.0.1:2277" &
    ${TERM} ${GEOMETRY} -e "ffplay -fflags nobuffer -i udp://127.0.0.1:2277" &
    ;;
  gnome-terminal) 
    ${TERM} ${GEOMETRY} -- ffmpeg -re -i RTI-vehicles-captioned-480.mp4 -g 15 -pix_fmt yuv420p -vcodec libx264 -preset ultrafast -tune zerolatency -f mpegts udp://127.0.0.1:2277 &

...
rtiComPubCtrl::rtiComPubCtrl(
    const std::string topicName, uint32_t opt_en, 
    dds::domain::DomainParticipant participant,
    dds::core::cond::WaitSet *waitset_ext,
    int(*rcv_fptr)(dds::sub::DataReader<cctypes::ccControl>))
    :
    data_topic(dds::core::null), pub_sample(), sub_sample(),
    data_wr(dds::core::null), data_rd(dds::core::null),
    waitset_int(), waitset()    
{ 
    // create topic
    data_topic = dds::topic::Topic<cctypes::ccControl>(participant, topicName);

    // publisher
    if (opt_en & CTRL_PUB_BE) {
        // DataWriter
#if 0   // bounded
        data_wr = dds::pub::DataWriter<cctypes::ccControl>(dds::pub::Publisher(participant), data_topic);
#else   // unbounded or bounded support
        dds::pub::qos::DataWriterQos writer_qos = dds::core::QosProvider::Default().datawriter_qos();
        writer_qos.policy<rti::core::policy::Property>().set({ "dds.data_writer.history.memory_manager.fast_pool.pool_buffer_max_size", "1048576" });
        data_wr = dds::pub::DataWriter<cctypes::ccControl>(dds::pub::Publisher(participant), data_topic, writer_qos);
#endif
    }
    ...
    if (opt_en & CTRL_SUB_BE) {
        // DataReader, ReadCondition, WaitSet
        data_rd = dds::sub::DataReader<cctypes::ccControl>(dds::sub::Subscriber(participant), data_topic);
        dds::sub::cond::ReadCondition data_readCond(
            data_rd, dds::sub::status::DataState::any(),
            [rcv_fptr, this](/* dds::core::cond::Condition condition */)
        {
...
// Built-in receive data handler (not used if rcv_fptr was provided at class instantiation)
int rtiComPubCtrl::rcv_dataProc(dds::sub::DataReader<cctypes::ccControl >& reader)
{
    int count = 0;
    dds::sub::LoanedSamples<cctypes::ccControl> samples = reader.take();
    for (const auto& sample : samples) {
        if (sample.info().valid()) {
            std::cout << "ComPubCtrl " << sample.data() << std::endl;
            sub_samples.push(sample);
            count++;
        }
    }
...
/*ci
 * \brief Publish the ccControl_ topic.
 */
void rtiComPubCtrl::publish(void)
{
    data_wr.write(pub_sample);
}

void rtiComPubCtrl::publish(cctypes::ccControl sample)
{
    data_wr.write(sample);
}
